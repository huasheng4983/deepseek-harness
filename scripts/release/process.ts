...
export function attempt(command: string, args: readonly string[], options: RunOptions = {}): CommandResult {
  // Windows resolves pnpm/npm through their .cmd shims, which spawnSync()
  // refuses without a shell (CVE-2024-27980 hardening) — same fix as apps/cli/src/plugin.ts.
 ...
 *
 * Windows resolves pnpm/npm through their .cmd shims, which spawn() refuses
 * without a shell (CVE-2024-27980 hardening) — same fix as `attempt`/`run`.
 * @param command - executable name.
 * @param args - command arguments.
 * @param options - working directory and environment.

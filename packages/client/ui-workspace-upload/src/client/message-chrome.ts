/** Shared time-label helpers for the user message actions row. */

import type { Translate } from '@deepseek-ai/dsh-client-ui-slots'

/** The date-template share of the workspace-upload dictionary the clock consumes. */
...
/**
 * Compact local timestamp for message actions. Same calendar day →
 * `HH:mm`; earlier this year → the `clock.md` date template + clock; other
 * years → the `clock.ymd` template + clock.
